...
/* MARK NAME Ramon Gonçalves Gonze */
...
{  int p[2], r;
  ...
  case ' ':
    ...
    if(execvp(ecmd->argv[0], ecmd->argv) < 0){
      fprintf(stderr, "%s: comando invalido\n", ecmd->argv[0]);
      exit(-1);
    }
  ...
  case '<':
    ...
    /* Permissões de rwx para o usuario e somente read para grupo/outros_usuários */
    ...
    if(r >= 0){
      ...
      fprintf(stderr, "erro ao tentar a abrir o arquivo\n");
      exit(-1);
    }
  ...
  case '|':
    ...
    if(r == 0){
      // Filho
      close(1); // Fecha a stdout
      dup2(p[1],1); // Duplica o lado de escrita do pipe para o fd=1
      close(p[0]);
      close(p[1]);
      runcmd(pcmd->left);
    }else{
      // Pai
      close(0); // Fecha a stdin
      dup2(p[0],0); // Duplica o lado de leitura do pipe para o fd=0
      close(p[0]);
      close(p[1]);
...
main(void)
{
  ...
  while(getcmd(buf, sizeof(buf)) >= 0){
    ...
    /* O comando cd não é suportado pelo exec. Isto acontece pois
     * o processo filho que o exec cria não consegue modificar o 
     * diretório atual do processo pai. A função chdir() do
     * segundo if verifica se o diretório existe ou não. Se a
     * função retorna um número negativo, o diretório é inválido. */

    if(buf[0] == 'c' && buf[1] == 'd' && buf[2] == ' '){
      buf[strlen(buf)-1] = 0;
      if(chdir(buf+3) < 0)
        fprintf(stderr, "%s: diretorio nao encontrado\n", buf+3);
      continue;
    }
